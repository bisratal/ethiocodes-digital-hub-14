...
import { Globe, Smartphone, Code, Cloud, Palette, Layers, ArrowRight, Zap, Shield, Clock, HeadphonesIcon } from "lucide-react";
  ...
  <main className="pt-16">
    ...
    <section className="section-padding">
      <div className="container-narrow space-y-20">
        {services.map((s, i) => (
          <ScrollReveal key={s.id} direction={i % 2 === 0 ? "right" : "left"} delay={0.1}>
            <div id={s.id} className={`flex flex-col md:flex-row gap-8 items-start ${i % 2 !== 0 ? "md:flex-row-reverse" : ""}`}>
              <div className="w-full md:w-1/3 flex justify-center">
                <div className="w-28 h-28 rounded-2xl gradient-accent flex items-center justify-center text-secondary-foreground shadow-elevated dark:glow-secondary">
                  {iconMap[s.icon]}
                </div>
              </div>
              <div className="w-full md:w-2/3">
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {s.technologies.map((t) => (
                    <span key={t} className="px-3 py-1 rounded-full text-xs font-medium bg-accent text-accent-foreground">{t}</span>
                  ))}
                </div>
                <div className="flex flex-wrap gap-4">
                  <Link to="/contact" className="inline-flex items-center gap-2 text-sm font-semibold text-secondary hover:underline">
                    Discuss This Service <ArrowRight size={14} />
                  </Link>
                  <Link to="/portfolio" className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground">
    ...
    <section className="section-padding bg-muted">
      <div className="container-narrow">
        <SectionHeading tag="Our Advantage" title="Why Choose EthioCodes" description="We don't just write code — we engineer solutions that move your business forward." />
    ...
    <section className="section-padding">
      <div className="container-narrow">
        <SectionHeading tag="How We Work" title="Our Development Process" description="A structured, transparent approach that keeps you informed at every stage." />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {processSteps.map((step, i) => (
            <ScrollReveal key={step.title} direction="up" delay={i * 0.08}>
              <div className="p-6 rounded-xl bg-card shadow-card relative dark:border dark:border-border">
                <div className="text-4xl font-extrabold text-accent dark:text-secondary/20 mb-3">{String(i + 1).padStart(2, "0")}</div>
    ...
    <section className="section-padding bg-muted">
      <div className="container-narrow">
        <SectionHeading tag="Technologies" title="Our Tech Stack" description="We use best-in-class tools and frameworks to build reliable, scalable solutions." />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {techStack.map((cat, i) => (
            <ScrollReveal key={cat.category} direction="up" delay={i * 0.1}>
              <div className="p-6 rounded-xl bg-card shadow-card dark:border dark:border-border">
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.tools.map((tool) => (
                    <span key={tool} className="px-3 py-1 rounded-full text-xs font-medium bg-accent text-accent-foreground">{tool}</span>
                  ))}
                </div>
              </div>
            </ScrollReveal>
    ...
    <section className="section-padding gradient-hero text-center">
      <ScrollReveal direction="scale" className="container-narrow">
        <h2 className="text-3xl md:text-4xl font-bold text-primary-foreground mb-4">Have a Project in Mind?</h2>
